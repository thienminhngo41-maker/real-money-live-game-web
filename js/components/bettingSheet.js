...
export function openBetting(){const overlay=document.querySelector('#bet-overlay');if(!overlay)return;window.scrollTo({top:0,behavior:'smooth'});if(overlay.parentElement!==document.body)document.body.appendChild(overlay);overlay.classList.add('show');overlay.setAttribute('aria-hidden','false')}
export function closeBetting(){const overlay=document.querySelector('#bet-overlay');if(!overlay)return;overlay.classList.remove('show');overlay.setAttribute('aria-hidden','true');window.setTimeout(()=>{const root=document.querySelector('#modal-root');if(root&&overlay&&!overlay.classList.contains('show'))root.appendChild(overlay)},280)}
